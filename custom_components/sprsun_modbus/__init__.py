...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def async_update_data():
        ...
        try:
            ...
            # ========== BULK READ 11a: Water Flow (372-377) - 6 registers ==========
            # Split into smaller chunks to reduce Modbus overload issues
            await asyncio.sleep(0.1)  # Small delay before critical read
            water_flow_regs = await hass.async_add_executor_job(
                client.read_holding_registers_bulk, REG_WATER_FLOW, 6, 0, slave_id, True  # raw=True
            )
            if water_flow_regs and len(water_flow_regs) == 6:
                # Water flow (372-373, 2 registers REAL/FLOAT32)
                water_flow_raw = registers_to_float32(water_flow_regs[0], water_flow_regs[1])
                data["water_flow"] = water_flow_raw if water_flow_raw is not None else None
                
                # Phase voltages (376-377, start of voltage readings)
                ...
                data["phase_voltage_a"] = to_signed_16bit(water_flow_regs[4])  # 376
                data["phase_voltage_b"] = to_signed_16bit(water_flow_regs[5])  # 377

            # ========== BULK READ 11b: Phase data (378-383) - 6 registers ==========
            await asyncio.sleep(0.1)  # Delay between reads
            phase_regs = await hass.async_add_executor_job(
                client.read_holding_registers_bulk, 377, 6, 0, slave_id, True  # raw=True
            )
            if phase_regs and len(phase_regs) == 6:
                def to_signed_16bit(val):
                    if val is None:
                        return None
                    if val > 32767:
                ...
                data["phase_voltage_c"] = to_signed_16bit(phase_regs[0])  # 378
                data["phase_current_a"] = to_signed_16bit(phase_regs[1])  # 379
                data["phase_current_b"] = to_signed_16bit(phase_regs[2])  # 380
                data["phase_current_c"] = to_signed_16bit(phase_regs[3])  # 381
                
                # Power_W (382-383, 2 registers REAL/FLOAT32)
                power_w_raw = registers_to_float32(phase_regs[4], phase_regs[5])
                data["power_w"] = power_w_raw if power_w_raw is not None else None

            # ========== BULK READ 11c: Unit Power and COP (384-389) - 6 registers ==========
            await asyncio.sleep(0.1)  # Delay before critical power/COP read
            unit_power_regs = await hass.async_add_executor_job(
                client.read_holding_registers_bulk, 383, 6, 0, slave_id, True  # raw=True
            )
            if unit_power_regs and len(unit_power_regs) == 6:
                # Total power consumption (384-385, 2 registers REAL/FLOAT32)
                total_power_raw = registers_to_float32(unit_power_regs[0], unit_power_regs[1])
                ...
                unit_power_raw = registers_to_float32(unit_power_regs[3], unit_power_regs[4])
                ...
                def to_signed_16bit(val):
                    ...
                    if val > 32767:
                        val = val - 65536
                    return float(val / 10)
                data["unit_cop"] = to_signed_16bit(unit_power_regs[5])
